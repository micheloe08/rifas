   ...
   @if($updateMode)
    ...
    <div class="dark:bg-gray-800 max-w-7xl2">
        <div class="mb-4 bg-orange-100 text-center">
            @if (session()->has('message'))
                <div class="alert alert-success">
                <span class="text-red-500 text-xl italic border-red-800">{{ session('message') }}</span>
                </div>
            @endif
        </div>
...
<div class="grid gap-12 mb-12 md:grid-cols-1">
    <button type="button" wire:click.prevent ="apartar()" class="text-white bg-gradient-to-r from-green-400 via-green-500 to-green-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-green-300 dark:focus:ring-green-800 shadow-lg shadow-green-500/50 dark:shadow-lg dark:shadow-green-800/80 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2">Apartar Boletos</button>
</div>
@if ($mostrar)
    <div class="list__buttons">
        <span class="font-medium"> Numeros Elegidos</span><br/>
        @foreach ($numerosElegidos as $item)
        <button  class="text-white bg-gradient-to-r from-green-400 via-green-500 to-green-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-green-300 dark:focus:ring-green-800 shadow-lg shadow-green-500/50 dark:shadow-lg dark:shadow-green-800/80 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2" wire:click = "borrar({{$item}})" ><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-3 h-3">
...
@endif
<div class="grid gap-12 mb-12 md:grid-cols-1">
    <button type="button" class="text-gray-900 bg-gradient-to-r from-lime-200 via-lime-400 to-lime-500 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-lime-300 dark:focus:ring-lime-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2">Seleccionar al azar</button>
</div>
<div class="flex gap-12 mb-12 cols-1">

    <label for="voice-search" class="sr-only">Search</label>
    <div class="relative w-full">
        <input type="text" class="bg-gray-50 border border-lime-500 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Busca tu número de la Suerte!" required wire:model.defer = "buscar" />
